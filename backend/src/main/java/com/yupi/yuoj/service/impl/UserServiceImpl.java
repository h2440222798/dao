...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword, String userName, String userEmail) {
        ...
        if (StringUtils.isNotBlank(userEmail) && !userEmail.contains("@")) {
    ...
    @Override
    public boolean updateConstitution(Long userId, Integer wood, Integer fire, Integer earth, Integer metal, Integer water) {
        if (userId == null || userId <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户不存在");
        }
        validateConstitutionScore("wood", wood);
        validateConstitutionScore("fire", fire);
        validateConstitutionScore("earth", earth);
        validateConstitutionScore("metal", metal);
        validateConstitutionScore("water", water);
        int total = wood + fire + earth + metal + water;
        if (total != 100) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "五行体质总分必须为 100");
        }
        return baseMapper.updateConstitutionByUserId(userId, wood, fire, earth, metal, water) > 0;
    }
    ...
    private void validateConstitutionScore(String element, Integer score) {
        if (score == null || score < 0 || score > 100) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, element + " 分值必须在 0~100 之间");
        }
    }
}
